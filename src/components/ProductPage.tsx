...
const ProductPage = () => {
  ...
  const parseProductIdFromUrl = (fullProductId: string) => {
    ...
    for (let i = parts.length - 1; i >= 0; i--) {
      const part = parts[i];
      if (
        /^\d+(gb|mm|tb)$/i.test(part) ||
        (/^\d+$/.test(part) && i > 0 && parts[i - 1].toLowerCase() !== "series")
      ) {
    ...
    if (potentialCapacityIndex !== -1) {
      ...
      if (
        ...
        !(
          ...
          /^\d+(gb|mm|tb)$/i.test(parts[parts.length - 1])
        )
      ) {
  ...
  const {
    ...
    error: overviewError,
  } = useQuery<ProductOverview | undefined>({
    ...
    queryFn: async () => {
      ...
      const found = allProducts?.find(
        (p: ProductOverview) => String(p.id) === productId
      );
  ...
  const {
    ...
    error: detailedError,
  } = useQuery<DetailedProduct | undefined>({
    queryKey: [productOverview?.category, productOverview?.id],
    queryFn: async () => {
      if (!productOverview?.category || !productOverview?.id) {
        return undefined;
      }
      const data = await fetchProductsByType(
        productOverview.category as ProductTypes
      );
      const found = data?.find(
        (p: DetailedProduct) => p.id === productOverview.id
      );
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return data
        ? (data as ProductOverview[])
            .filter((p: ProductOverview) => {
              return (
                p.category === productOverview.category &&
                parseProductIdFromUrl(p.id ?? "").namespaceId !==
                  parseProductIdFromUrl(productOverview.id ?? "").namespaceId
              );
            })
            .slice(0, 4)
            .map(transformProductOverview)
        : undefined;
    },
    enabled: !!productOverview?.category && !!productOverview?.id,
    staleTime: 3000,
  ...
  const activeColor = useMemo(() => {
    const match = detailedProduct?.colorsAvailable?.find(
      (c: string) =>
        formatStringToUrlParam(c) === formatStringToUrlParam(urlColor)
    );
  ...
  const activeCapacity = useMemo(() => {
    const match = detailedProduct?.capacityAvailable?.find(
      (c: string) =>
        formatStringToUrlParam(c) === formatStringToUrlParam(urlCapacity)
    );
  ...
  const collectTechSpecs = (product: DetailedProduct) => {
    ...
    if (product.resolution)
      specs.push({ label: "Resolution", value: product.resolution });
    if (product.processor)
      specs.push({ label: "Processor", value: product.processor });
    if (product.ram) specs.push({ label: "RAM", value: product.ram });
    if (product.capacity)
      specs.push({ label: "Built in memory", value: product.capacity });
    if (product.camera) specs.push({ label: "Camera", value: product.camera });
    if (product.zoom) specs.push({ label: "Zoom", value: product.zoom });
    if (product.cell)
      specs.push({ label: "Cell", value: product.cell.join(", ") });
    return specs;
  };
      ...
      <nav className="text-sm mb-4">
        <ol className="list-none p-0 flex w-full flex-wrap">
          ...
          <li className="flex items-center min-w-0 flex-grow">
            <span className="text-white truncate">{productOverview.name}</span>
          </li>
        </ol>
      </nav>
              ...
              <img
                ...
                className={`w-20 h-20 object-contain border-2 ${
                  mainImage === `${image}`
                    ? "border-blue-500"
                    : "border-transparent"
                } cursor-pointer rounded-md flex-shrink-0`}
        ...
        <div className="w-full md:w-1/2">
                ...
                (color: string, index: number) => (
                  <div
                    ...
                    className={`w-8 h-8 rounded-full border-2 ${
                      activeColor === formatStringToUrlParam(color)
                        ? "border-blue-500"
                        : "border-gray-600"
                    } cursor-pointer`}
                  ...
                  return (
                    <button
                      ...
                      className={`px-4 py-2 rounded-md border ${
                        activeCapacity ===
                        formatStringToUrlParam(capacity).toUpperCase()
          ...
          <div className="flex gap-4 mb-8">
            {currentItem ? (
              <Button
                asChild
                className="flex-grow bg-secondary group"
                disabled={isCartPending}
              >
                <Link
                  to="/cart"
                  className="flex items-center justify-center gap-x-1 py-3"
                >
                  Go to cart
            ...
            <Button
              onClick={() =>
                handleToggleFavorites(detailedProduct as unknown as Product)
              }
              variant="secondary"
